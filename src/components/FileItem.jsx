...
const FileItem = ({ file, index, onRemove, viewMode = 'list', className = '' }) => {
  if (viewMode === 'grid') {
    return (
      <div className={`relative flex flex-col p-3 bg-white border border-gray-200 rounded-md hover:border-gray-300 transition-colors ${className}`}>
        <div className="w-full aspect-[5/3] bg-gradient-to-br from-red-50 to-red-100 border-2 border-red-200 rounded-md flex items-center justify-center mb-2 shadow-sm" aria-hidden="true">
        ...
        {onRemove && (
          <button
            ...
            type="button"
          >
            <div className="w-3.5 h-3.5 text-white" aria-hidden="true">{icons.close}</div>
          </button>
        )}
      </div>
    )
